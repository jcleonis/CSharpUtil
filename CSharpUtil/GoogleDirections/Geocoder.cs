...
namespace GoogleDirections
{
    ...
    public sealed class Geocoder : HttpWebService
    {
        ...
        {
            //MessageBox.Show("formatei a url e vou chamar API: " + string.Format("{1}&address={0}&sensor=false", address, BaseUrl()), "csharp.dll");
            var response = MakeRequest(
              string.Format("{1}&address={0}&sensor=false", address, BaseUrl()));
            //MessageBox.Show("Chamei e peguei o Resultado: "+ response, "csharp.dll");
            var responseXml = new XmlDocument();
            responseXml.LoadXml(response);
